...
class transformer(object):
    ...
    def __init__(self, name, embedding_len, bert_config_path, max_sent_num,
                 batch_size, train):
        ...
        with tf.device('/gpu:0'):
            ...
            self.first_num_sents = tf.placeholder(dtype=tf.int32,
                                                  shape=[batch_size],
                                                  name="first_num_sent")
            self.second_num_sents = tf.placeholder(dtype=tf.int32,
                                                   shape=[batch_size],
                                                   name="second_num_sent")
            ...
            with tf.variable_scope('separator'):
                self._separator = tf.get_variable(
                    'separator',
                    shape=[1, self._embedding_len],
                    dtype=tf.float32)
    ...
    def make_graph(self):
        ...
        self.bert_model = BertModel(config=self._config,
                                    is_training=False,
        ...
        with tf.device('/gpu:0'):
            ...
            embeddings = tf.reduce_mean(
                self.bert_model.get_all_encoder_layers()[-1], axis=1)
            ...
            tiled_separator = tf.tile(
                self._separator,
                tf.constant([self._batch_size, 1], dtype=tf.int32))
            separator_indices = tf.concat(
                (tf.expand_dims(tf.range(self._batch_size), axis=1),
                 tf.expand_dims(self.first_num_sents, axis=1)),
                axis=1)
            ...
            bert_res = tf.scatter_nd(
                all_indices,
                all_embeddings,
                shape=tf.constant([
                    self._batch_size, 2 * self._max_sent_num + 1,
                    self._embedding_len
                ],
                                  dtype=tf.int32))

            #making padding for transformer
            transformer_padding = tf.dtypes.cast(
                tf.bitwise.invert(
                    tf.sequence_mask(self.first_num_sents +
                                     self.second_num_sents + 1,
                                     maxlen=2 * self._max_sent_num + 1,
                                     dtype=tf.int32)), tf.float32)

            #making attenstion bias
            attention_bias = tf.expand_dims(tf.expand_dims(transformer_padding *
                                                           _NEG_INF,
                                                           axis=1),
                                            axis=1)
        ...
        with tf.variable_scope("transformer"):
            transformer_res = self._transformer.encode(
                (bert_res, transformer_padding), attention_bias)
            ...
            with tf.variable_scope("feedforward"):
                ...
                self.loss = tf.reduce_mean(
                    tf.nn.softmax_cross_entropy_with_logits_v2(
                        logits=self.final_logits, labels=self.target))

    def variables(self):
        """
        :return: variables of the model, excluding the BERT model variables
        """
        ret = tf.get_collection(tf.GraphKeys.GLOBAL_VARIABLES,
                                scope='transformer')
        ret = ret + tf.get_collection(tf.GraphKeys.GLOBAL_VARIABLES,
                                      scope='feedforward')
        ret = ret + tf.get_collection(tf.GraphKeys.GLOBAL_VARIABLES,
                                      scope='separator')
