...
class transformer(object):
    ...
    def __init__(self, name, embedding_len, max_sent_num, batch_size, train):
        ...
        self.inputs = tf.placeholder(
            dtype=tf.float32,
            shape=[batch_size, 2 * max_sent_num + 1, embedding_len],
            name="inputs")
        ...
        with tf.variable_scope('separator'):
            self._separator = tf.get_variable('separator',
    ...
    def make_graph(self):
        tiled_separator = tf.tile(
            self._separator, tf.constant([self._batch_size, 1], dtype=tf.int32))
        ...
        updates = tf.scatter_nd(separator_indices,
                                tiled_separator,
                                shape=tf.constant([
                                    self._batch_size, 2 * self._max_sent_num +
                                    1, self._embedding_len
                                ],
                                                  dtype=tf.int32))
        ...
        transformer_padding = tf.dtypes.cast(
            tf.bitwise.invert(
                tf.sequence_mask(self.first_num_sents + self.second_num_sents +
                                 1,
        ...
        with tf.variable_scope("transformer"):
            transformer_res = self._transformer.encode(
                (transformer_inputs, transformer_padding), attention_bias)
