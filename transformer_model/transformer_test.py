...
def test(files , batch_size ,max_sent_num , embedding_len, model_path):
    ...
    test_batches = batch_generator(files, batch_size=batch_size , max_sent_num=max_sent_num ,
    ...
    with tf.Session(config=config) as sess:
        ...
        for batch_num , (first_num , second_num , inputs  , labels) in enumerate(test_batches):
            count+=1
...
if __name__=='__main__':
    ...
    test(files , batch_size=64  , max_sent_num = 37, embedding_len = 768 , model_path="./bert_transformer_2adam001.ckpt")
