...
class CNNEncoder3(Model):
    ...
    def __init__(self, name, batch_size, paragraph_len, sentence_len, dim, embedding_table,
        ...
        self.model.add(layers.Reshape(name=name + "/Reshape1",
                                      target_shape=(batch_size * paragraph_len, sentence_len, dim)))

        self.model.add(CNNEncoder2(name=name + "/SentEncoder", embedding_table=embedding_table,
                                   hidden_len=hidden_len, num_layer=sent_num_layer, kernel_size=kernel_size,
                                   filters=filters, padding=padding, pool_size=pool_size, dropout=dropout))

        self.model.add(layers.Reshape(name=name + "/Reshape2",
                                      target_shape=(batch_size, paragraph_len, dim)))

        self.model.add(CNNEncoder1(name=name + "/ParEncoder", num_layer=par_num_layer,
