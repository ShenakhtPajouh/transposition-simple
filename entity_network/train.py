...
def get_loss(inputs, model, y_true, mask):
    y_pred = model(inputs)
    return tf.reduce_mean(tf.multiply(losses.categorical_crossentropy(one_hot(y_true), y_pred), mask))
...
def train(model, learning_rate, num_epochs, save_path, batch_size, train_steps = 50, validation_steps = 1000):
    ...
    for batch in batchgenerator:  
        ...
        loss_avg(loss)
        train_accuracy(np.argmax(np.apply_along_axis(np.bincount, 1, np.argmax(outputs.numpy(), axis = 2)), axis = 1), labels)
        step += 1
        ...
        if step % train_steps == 0:
            ...
            print("Step " + str(step) + ": " + "Train Loss= " + str(loss_avg.result().numpy()) + ", Train Accuracy= " + str(train_accuracy.result().numpy()))
            loss_avg = tfe.metrics.Mean()
            train_accuracy = tfe.metrics.Accuracy()
            ...
            if step % validation_steps == 0:
                ...
                for validation in validationgenerator:
                    ...
                    validation_accuracy(np.argmax(np.apply_along_axis(np.bincount, 1, np.argmax(model(x).numpy(), axis = 2)), axis = 1), y)
                

                validation_accuracy_results.append(validation_accuracy.result().numpy())
                print("Validation Accuracy= " + str(validation_accuracy.result().numpy()))
