...
def train(dataset, batch_size, max_sent_len, max_sent_num, embedding_len,
          bert_vocab_path, bert_config_path, bert_checkpoint, epochs,
          learning_rate, lr_decay, validation_split):
    ...
    model = transformer("transformer1",
                        embedding_len,
                        bert_config_path,
                        max_sent_num,
                        batch_size,
    ...
    train_batches = batch_generator(train,
                                    batch_size=batch_size,
                                    max_sent_len=max_sent_len,
                                    bert_vocab_path=bert_vocab_path,
                                    epochs=epochs)

    validation_batches = batch_generator(validation,
                                         batch_size=batch_size,
                                         max_sent_len=max_sent_len,
                                         bert_vocab_path=bert_vocab_path)
    ...
    with tf.Session(config=config) as sess:
        ...
        saver = tf.train.Saver({
            v.name: v for v in tf.get_collection(tf.GraphKeys.GLOBAL_VARIABLES,
                                                 scope='bert')
        })
        ...
        for batch_num, (first_num, second_num, indices, inputs, mask,
                        labels) in enumerate(train_batches):
            count += 1
            _, loss = sess.run(
                [train_opt, model.loss],
                feed_dict={
            ...
            if (batch_num % 5000 == 0):
                val_count = 0
                for batch_num, (first_num, second_num, indices, inputs, mask,
                                labels) in enumerate(validation_batches):
                    val_count += 1
                ...
                print(val_count)
                print(batch_size)
                print("Step: " + batch_num + ":validation accuracy = " +
                      str(correct / (batch_size * val_count)))
...
if __name__ == '__main__':
    ...
    train(all_paragraphs[:1000], 32, 300, 37, 768,
          "/mnt/storage1/Data/BERT/uncased_L-12_H-768_A-12*/vocab.txt",
          "/mnt/storage1/Data/BERT/uncased_L-12_H-768_A-12*/bert_config.json",
          "/mnt/storage1/Data/BERT/new/bert_vars.ckpt", 100, 0.01, 0.01, 0.1)
