...
def validation_generator(batch_size):
    ...
    for i in range(2):
        for j in range(3):
            e, mask = f(pairs_encoded[j][i], pairs_encoded[j][i^1], pairs_encoded[j][2+i], pairs_encoded[j][2+(i^1)], len(pairs_encoded[j][3]), max_len, dim)
            encoded = np.append(encoded, e, axis = 0)
            masks = np.append(masks, mask, axis = 0)
            labels = np.append(labels, np.tile([i^1], len(pairs_encoded[j][3])))
    ...
    for i in range(0, n, batch_size):
        if i + batch_size > n:
            break
        yield (encoded[i:i + batch_size], masks[i:i + batch_size], keys[i:i + batch_size], keys_mask[i:i + batch_size], labels[i:i + batch_size])
...
def batch_generator(batch_size):
    for epoch in range(10):
        for b in range(9):
            pairs_encoded = []
            for i in range(2):
                path = '../paragraph_pairs_encoded' + str(2*b + 1 + i) + '.pkl'
                with open(path, 'rb') as pkl:
                    pairs_encoded.append(pickle.load(pkl))
