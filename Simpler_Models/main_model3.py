...
class main_model(tf.keras.Model):
    ...
    def __init__(self,
        ...
        self._name = name
        self._rnn_encoder = sent_encoder(self._name + '/LSTM_encoder',
                                         embedding_table, hidden_len)
        self._cnn_encoder = CNNEncoder1(self._name + '/GLU_CNN_encoder',
                                        filters=filters)
    ...
    def call(self, inputs):
        ...
        first_indices = tf.concat((range, tf.dtypes.cast(first, tf.int32)),
                                  axis=2)
